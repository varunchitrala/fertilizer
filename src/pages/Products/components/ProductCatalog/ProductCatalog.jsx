...
function ProductCard({ product, index, onOpen }) {
  return (
    <ScrollReveal distance={24} delay={(index % 2) * 70}>
      <article className="product-card">
        <button className="product-card-open" type="button" onClick={() => onOpen(product, index)} aria-label={`View ${product.name} details`}>
          ...
          <div className="product-card-info">
            ...
            <strong>View product details <b>→</b></strong>
          </div>
        </button>
      </article>
    </ScrollReveal>
      ...
      <div className="container">
        <header className="products-catalog-header">
          <div className="products-catalog-heading">
            ...
            <p>Explore the complete Agripex product range. Choose a product to view the information currently available for it.</p>
          </div>
        ...
        <div className="products-catalog-toolbar">
          ...
          <span>25 PRODUCTS <b>·</b> VIEW DETAILS</span>
        </div>

        <div className="products-grid">
          {products.map((product, index) => <ProductCard key={`${product.name}-${index}`} product={product} index={index} onOpen={openProduct} />)}
        </div>
      </div>
