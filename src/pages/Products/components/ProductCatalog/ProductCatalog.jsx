...
const products = [
  { name: "Calcium Nitrate", description: "Calcium and nitrogen fertilizer for crop development and balanced nutrition.", image: "/a1.jpg", details: { overview: "Calcium Nitrate fortified with Magnesium is a highly soluble liquid fertilizer providing nitrogen, calcium, and magnesium. It supports healthy vegetative growth, strong cell structure, and improved fruit quality, enhanced by efficient nutrient availability to the plant.", features: [{number:"01",title:"EA² Technology",text:"Enhanced nutrient uptake and increased efficacy, with improved nutrient translocation and faster results."},{number:"02",title:"S² Technology",text:"Concentration beyond what is normally achievable in liquid formulations, with improved nutrient translocation and faster results."},{number:"03",title:"High Quality Raw Materials",text:"Designed around quality raw materials to support maximum efficacy and safety."},{number:"04",title:"Liquid Form",text:"Easy to measure and convenient to use."},{number:"05",title:"Highly Concentrated",text:"Reduced doses and increased efficacy resulting in lower application costs."},{number:"06",title:"Enhanced Mix Profile",text:"Facilitates combined application for reduced application cost."},{number:"07",title:"Stability",text:"Certified manufacturing processes and testing support a long shelf life."}], application:["Fertigation","Drip irrigation","Foliar Spray"], dosage:["Foliar Spray: 3 ml to 5 ml / Litre water","Drip Irrigation: 3000 - 5000 ml / per acre"], crops:["Grapes","Pomegranate","Banana","Cotton","Tomato","Onion","Sugarcane","Ginger","Turmeric","Watermelon"], form:"Liquid fertilizer · 1 litre", specifications:[{label:"Base",value:"Liquid Gel"},{label:"Total Nitrogen",value:"15.50% Wt/Wt · 20.50% Wt/Vol"},{label:"Total Calcium (Ca)",value:"19.00% Wt/Wt · 25.10% Wt/Vol"},{label:"Total Magnesium (Mg)",value:"02.00% Wt/Wt · 2.60% Wt/Vol"},{label:"Trace Elements (EDTA)",value:"Yes"},{label:"pH",value:"5.5-6.5"},{label:"Specific Gravity",value:"1.32-1.35 @18°"}], storage:"Store in cool place, away from direct sunlight & heat.", caution:"Keep out of reach of children. Shake well before use. For agriculture use only.", manufacturer:"AGRIPEX PESTICIDE FERTILIZER AND CHEMICAL PRIVATE LIMITED" } },
...
function ProductCard({ product, index, onOpen }) {
  return (
    <ScrollReveal distance={28} delay={(index % 3) * 60}>
      <article className="product-card">
        <div className="product-card-image">
          <img src={product.image} alt={product.name} loading="lazy" />
          <span>{String(index + 1).padStart(2, "0")}</span>
          <div className="product-card-image-label">AGRIPEx</div>
        </div>
        <div className="product-card-body">
          <div className="product-card-type">{index < 19 ? "Crop nutrition" : "Agricultural input"}</div>
          <h2>{product.name}</h2>
          <p>{product.description}</p>
          <button type="button" onClick={() => onOpen(product, index)}>
...
function ProductCatalog() {
  ...
  const openProduct = (product, index) => setSelectedProduct({ ...product, index });
    ...
    <section className="products-catalog" id="products-catalog">
      <div className="container">
        <header className="products-catalog-header">
          <div>
            <div className="products-catalog-label"><span>01</span> Product catalogue</div>
            <h2>Our complete <em>product range.</em></h2>
            <p>Browse the Agripex range and open any product to view the available technical information.</p>
          </div>
          <div className="products-catalog-count"><strong>25</strong><span>Products</span></div>
        </header>

        <div className="products-catalog-rule" />
        <div className="products-grid">
          {products.map((product, index) => (
            <ProductCard key={`${product.name}-${index}`} product={product} index={index} onOpen={openProduct} />
          ))}
        </div>
      </div>
