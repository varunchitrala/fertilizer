        ...
        <section className="products-hero">
          ...
          <div className="container products-hero-inner">
            <ScrollReveal distance={35}>
              <span className="products-eyebrow">Agripex / Product Collection</span>
              <h1>Agricultural products,<br /><em>designed for growth.</em></h1>
              <p>Explore the Agripex product collection, with agricultural inputs developed to support crop nutrition, protection, and productivity.</p>
              <a className="products-hero-link" href="#products-catalog">Explore products <span>↓</span></a>
            </ScrollReveal>
          </div>
          <div className="products-hero-meta container">
            <span>01 / COLLECTION</span><span>AGRIPEX</span><span>{products.length} PRODUCTS</span>
          </div>
        </section>
        ...
        <section className="products-catalog" id="products-catalog">
          <div className="container">
            <div className="products-heading">
              <div><span className="products-section-number">01</span><span className="products-section-label">Our product range</span></div>
              <p>Fertilizers · Nutrition · Crop Care</p>
            </div>

            <div className="products-grid">
              {products.map((product, index) => (
                <ScrollReveal className="product-card" distance={35} delay={(index % 3) * 70} key={product.name}>
                  <article className="product-card-inner">
                    <div className="product-card-image">
                      <img src={product.image} alt={product.name} loading="lazy" />
                      <span className="product-card-number">{String(index + 1).padStart(2, "0")}</span>
                      <span className="product-card-corner">AGRIPEX</span>
                    </div>
                    <div className="product-card-content">
                      <span className="product-card-category">AGRICULTURAL INPUT</span>
                      <h2>{product.name}</h2>
